...
const LEGACY_STORAGE_KEYS = {
  ...
  openingBalance: ["finora-opening-balance"],
  budgets: ["finora-custom-budgets", "finora-budgets"],
  recurringRules: ["finora-recurring-rules"],
  theme: ["finora-theme"],
};
...
const elements = {
  ...
  emptyAddButton: document.getElementById("empty-add"),
  settingsButton: document.getElementById("settings-button"),
  changeBalanceButton: document.getElementById("change-balance-button"),
  addBudgetButton: document.getElementById("add-budget-button"),
  installButton: document.getElementById("install-button"),
  transactionClose: document.getElementById("transaction-close"),
  ...
  transactionBudget: document.getElementById("transaction-budget"),
  notes: document.getElementById("notes"),
  makeRecurring: document.getElementById("make-recurring"),
  recurringOptions: document.getElementById("recurring-options"),
  frequency: document.getElementById("frequency"),
  recurringEnd: document.getElementById("recurring-end"),
  ...
  resetTransactions: document.getElementById("reset-transactions"),
  resetBudgets: document.getElementById("reset-budgets"),
  ...
  budgetSave: document.getElementById("budget-save"),
  recurringId: document.getElementById("recurring-id"),
  recurringDescription: document.getElementById("recurring-description"),
  ...
  recurringBudget: document.getElementById("recurring-budget"),
  recurringNext: document.getElementById("recurring-next"),
  recurringRuleEnd: document.getElementById("recurring-rule-end"),
  ...
  heroCurrency: document.getElementById("hero-currency"),
  transactionPrefix: document.getElementById("transaction-prefix"),
  settingsPrefix: document.getElementById("settings-prefix"),
  budgetPrefix: document.getElementById("budget-prefix"),
  recurringPrefix: document.getElementById("recurring-prefix"),
};

let transactions = ensureArray(
  loadJSON(STORAGE_KEYS.transactions, []),
);

let budgets = ensureArray(
  loadJSON(STORAGE_KEYS.budgets, []),
);

let recurringRules = ensureArray(
  loadJSON(STORAGE_KEYS.recurringRules, []),
);
...
let openingBalance = Number(
  localStorage.getItem(STORAGE_KEYS.openingBalance) || 0,
);
...
if (!CURRENCY_SYMBOLS[selectedCurrency]) {
...
function migrateLegacyStorage() {
  Object.entries(STORAGE_KEYS).forEach(
    ([storageName, newKey]) => {
      ...
      const legacyKeys =
        LEGACY_STORAGE_KEYS[storageName] || [];
...
function normalizeTransactions(values) {
  return values
    ...
    .map((item) => ({
      ...
      date: isValidDateString(item.date)
        ? item.date
        : getTodayString(),
...
function normalizeRecurringRules(values) {
  return values
    .filter(
      (item) =>
        item &&
    ...
    .map((item) => {
      const startDate = isValidDateString(
        item.startDate,
      )
        ? item.startDate
        : getTodayString();
...
function getCategoryDetails(categoryName) {
  ...
  return (
    CATEGORY_STYLES[
      index % CATEGORY_STYLES.length
    ] ||
    CATEGORY_STYLES[
      CATEGORY_STYLES.length - 1
    ]
  );
}
...
function showStatus(
  ...
  if (
    ...
    elements.statusAction.textContent =
      actionText;

    elements.statusAction.classList.remove(
      "hidden",
    );

    elements.statusAction.onclick =
      action;
  } else {
    elements.statusAction.classList.add(
      "hidden",
    );
...
function resetTransactionForm() {
  ...
  elements.transactionSave.textContent =
    "Save transaction";
  ...
  elements.makeRecurring.checked =
    false;
  ...
  elements.recurringOptions.classList.add(
    "hidden",
  );
...
function resetBudgetForm() {
  ...
  elements.budgetSave.textContent =
    "Save budget";
...
function openEditRecurringModal(id) {
  ...
  elements.recurringError.textContent =
    "";
...
function handleBudgetSubmit(event) {
  ...
  const name =
    elements.budgetName.value.trim();
...
function handleRecurringSubmit(
  ...
  const description =
    elements.recurringDescription.value.trim();
...
function renderTransactions() {
  ...
  elements.transactionEmpty.classList.toggle(
    "hidden",
  ...
  filteredTransactions.forEach(
    (transaction) => {
      ...
      const recurringBadge =
        transaction.generatedByRecurringRule
          ? `
      ...
      elements.transactionList.appendChild(
        item,
      );
    },
  );
...
function renderCategoryBreakdown() {
  ...
  sortedCategories.forEach(
    ([categoryName, amount]) => {
      ...
      elements.categoryBreakdown.appendChild(
        row,
      );
    },
  );
...
function renderBudgets() {
  ...
  budgets.forEach((budget) => {
    ...
    elements.budgetList.appendChild(
      item,
    );
  });
...
function renderRecurringRules() {
  ...
  recurringRules
    ...
    .forEach((rule) => {
      ...
      elements.recurringList.appendChild(
        item,
      );
    });
...
elements.addTransactionButton.addEventListener(
  "click",
...
elements.changeBalanceButton.addEventListener(
  "click",
  openSettingsModal,
);

elements.addBudgetButton.addEventListener(
  "click",
  openNewBudgetModal,
);
...
elements.transactionClose.addEventListener(
  "click",
  () => {
    closeModal(
      elements.transactionModal,
...
elements.transactionCancel.addEventListener(
  "click",
  () => {
    closeModal(
      elements.transactionModal,
...
elements.settingsCancel.addEventListener(
  "click",
  () => {
    closeModal(
      elements.settingsModal,
...
elements.budgetClose.addEventListener(
  "click",
  () => {
    closeModal(
...
elements.budgetCancel.addEventListener(
  "click",
  () => {
    closeModal(
      elements.budgetModal,
...
elements.recurringClose.addEventListener(
  "click",
  () => {
    closeModal(
      elements.recurringModal,
...
elements.recurringCancel.addEventListener(
  "click",
  () => {
    closeModal(
      elements.recurringModal,
...
elements.recurringForm.addEventListener(
  "submit",
  handleRecurringSubmit,
);

elements.makeRecurring.addEventListener(
  "change",
  () => {
    elements.recurringOptions.classList.toggle(
      "hidden",
...
elements.exportCsv.addEventListener(
  "click",
  exportTransactionsToCSV,
);

elements.importCsv.addEventListener(
  "click",
  () => {
...
elements.csvInput.addEventListener(
  "change",
  (event) => {
    const file =
...
elements.addCategory.addEventListener(
  "click",
  addCustomCategory,
);

elements.newCategory.addEventListener(
  "keydown",
  (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
...
elements.importBackup.addEventListener(
  "click",
  () => {
...
elements.backupInput.addEventListener(
  "change",
  (event) => {
    const file =
      event.target.files?.[0];
...
elements.resetTransactions.addEventListener(
  "click",
  resetTransactionsData,
);

elements.resetBudgets.addEventListener(
  "click",
  resetBudgetsData,
);

elements.resetAll.addEventListener(
  "click",
  resetAllData,
);

elements.statusClose.addEventListener(
  "click",
  () => {
    elements.statusBanner.classList.add(
      "hidden",
...
(() => {
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      /*
       * Trackpads already provide smooth native scrolling.
       */
      if (
        event.deltaMode === 0 &&
